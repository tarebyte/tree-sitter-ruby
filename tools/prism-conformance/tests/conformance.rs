...
const VALID_FIXTURES: &str = "test/prism/valid";
const INVALID_FIXTURES: &str = "test/prism/invalid";
...
fn parse_fixture_directory(root: &Path, expect_prism_success: bool) {
    ...
    for path in ruby_files(root) {
        ...
        let prism_errors = prism_diagnostics(&source);
        let tree_sitter_errors = tree_sitter_errors(&mut parser, &source);

        if expect_prism_success {
            if !prism_errors.is_empty() {
                failures.push(format!(
                    "{}: Prism unexpectedly rejected source:\n  {}",
                    path.display(),
                    prism_errors.join("\n  ")
                ));
            } else if !tree_sitter_errors.is_empty() {
                failures.push(format!(
                    "{}: Tree-sitter rejected Prism-valid source:\n  {}",
                    path.display(),
                    tree_sitter_errors.join("\n  ")
                ));
            }
        } else {
            if prism_errors.is_empty() {
                failures.push(format!(
                    "{}: Prism unexpectedly accepted invalid source",
                    path.display()
                ));
            }
            if tree_sitter_errors.is_empty() {
                failures.push(format!(
                    "{}: Tree-sitter accepted Prism-invalid source without recovery nodes",
                    path.display()
                ));
            }
        }
...
#[test]
fn prism_valid_fixtures_parse_without_recovery() {
    parse_fixture_directory(&repository_path(VALID_FIXTURES), true);
}

#[test]
fn prism_invalid_fixtures_require_recovery() {
    parse_fixture_directory(&repository_path(INVALID_FIXTURES), false);
}
